...
impl fmt::Display for GenerateDigitsError {
    fn fmt(&self, formatter: &mut fmt::Formatter) -> fmt::Result {
        match self {
            GenerateDigitsError::InvalidLength(length) => {
...
pub fn generate_digits(length: &u64) -> Result<u64, GenerateDigitsError> {
    if length <= &0 {
        return Err(
            GenerateDigitsError::InvalidLength(*length)
        );
    ...
    let value = thread_rng().gen_range(minimum..maximum);
